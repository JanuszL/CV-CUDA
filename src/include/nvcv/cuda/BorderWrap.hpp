...
public:
    ...
    explicit __host__ BorderWrapImpl(const ITensorDataPitchDevice &tensor)
        : m_tensorWrap(tensor)
    {
        int j = 0;
#pragma unroll
...
public:
     ...
     *
     * @param[in] tensor A \ref ITensorDataPitchDevice object to be wrapped
     * @param[in] borderValue The border value is ignored in non-constant border types
     */
    explicit __host__ BorderWrap(const ITensorDataPitchDevice &tensor, ValueType borderValue = {})
        : Base(tensor)
    {
    }
