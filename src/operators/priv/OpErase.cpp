                       ...
                       unsigned int seed) const
{
    ...
    {
        throw cv::Exception(cv::Status::ERROR_INVALID_ARGUMENT, "anchor must be cuda-accessible, pitch-linear tensor");
    }

    auto *erasingData = dynamic_cast<const cv::ITensorDataStridedCuda *>(erasing.exportData());
    if (erasingData == nullptr)
    {
        throw cv::Exception(cv::Status::ERROR_INVALID_ARGUMENT, "erasing must be cuda-accessible, pitch-linear tensor");
    }

    auto *valuesData = dynamic_cast<const cv::ITensorDataStridedCuda *>(values.exportData());
    if (valuesData == nullptr)
    {
        throw cv::Exception(cv::Status::ERROR_INVALID_ARGUMENT, "values must be cuda-accessible, pitch-linear tensor");
    }

    auto *imgIdxData = dynamic_cast<const cv::ITensorDataStridedCuda *>(imgIdx.exportData());
    if (imgIdxData == nullptr)
    {
        throw cv::Exception(cv::Status::ERROR_INVALID_ARGUMENT, "imgIdx must be cuda-accessible, pitch-linear tensor");
    }
