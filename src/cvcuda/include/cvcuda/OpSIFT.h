 ...
 *
 * @param [out] featDescriptors Output tensor with features descriptors.  The expected layout is [NM] or [NMC]
 *                              meaning a rank-2 or rank-3 tensor with first dimension as number of samples N,
 *                              second dimension M as maximum number of features to be found, and a potential last
 *                              dimension C with number of feature descriptors.
 *                              + It must have the same number of samples N as input tensor.
 ...
 *                              + It must have U64 data type with C=2 or 2U64 data type with C=1 (or C not present in
 *                                tensor) to store each 128-Byte feature descriptor.
 *
