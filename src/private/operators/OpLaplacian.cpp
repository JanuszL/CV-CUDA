                           ...
                           const float scale, const NVCVBorderType borderMode) const
{
    ...
    leg::helpers::CheckOpErrThrow(m_legacyOp->infer(*inData, *outData, ksize, scale, borderMode, stream));
}
                           ...
                           const cv::ITensor &ksize, const cv::ITensor &scale, NVCVBorderType borderMode) const
{
    ...
    leg::helpers::CheckOpErrThrow(
        m_legacyOpVarShape->infer(*inData, *outData, *ksizeData, *scaleData, borderMode, stream));
}
