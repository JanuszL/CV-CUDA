 ...
 */
template<typename T>
__device__ T fetch(T *shared, const Ptr2dNHWC <T> src, int batchIdx, int h, int w, int c,
 ...
 */
template<typename T>
__global__ void median(const Ptr2dNHWC <T> src, Ptr2dNHWC <T> dst,
...
template<typename T>
__global__ void medianForSmallKernel(const Ptr2dNHWC <T> src, Ptr2dNHWC <T> dst,
                                     const int kWidth, const int kHeight)
{
    int tx = threadIdx.x, ty = threadIdx.y;
    int blockX = blockIdx.x * blockDim.x;
...
template<typename T>
void median(const void *input, void *output, const DataShape inputShape,
            int kWidth, int kHeight, cudaStream_t stream)
{
    Ptr2dNHWC <T> src(inputShape.N, inputShape.H, inputShape.W, inputShape.C, (T *) input);
    Ptr2dNHWC <T> dst(inputShape.N, inputShape.H, inputShape.W, inputShape.C, (T *) output);
...
int MedianBlur::infer(const void *const *inputs, void **outputs, void *workspace,
                      cv::Size ksize, DataShape inputShape, DataFormat format,
                      DataType data_type, cudaStream_t stream)
{
    ...
    if(!(ksize.width > 0 && ksize.width % 2 == 1 && ksize.height > 0 && ksize.height % 2 == 1))
    {
        LOG_ERROR("Invalid ksize " << ksize.width << " " << ksize.height);
    ...
    typedef void (*median_t)(const void *input, void *output, DataShape inputShape,
                             int kWidth, int kHeight, cudaStream_t stream);
    ...
    funcs[data_type](inputs[0], outputs[0], inputShape, ksize.width, ksize.height, stream);
    return 0;
}
