           ...
           const float4 &borderValue)
{
    ...
    const float2 mapAS{0.f, 0.f}, srcAS{0., 0.f}, mapBVal{0.f, 0.f};

    const ValueType srcBVal
        = cuda::DropCast<cuda::NumElements<ValueType>>(cuda::StaticCast<cuda::BaseType<ValueType>>(borderValue));
    ...
    for (int z = 0; z < dstShape.z; ++z)
    {
        for (int y = 0; y < dstShape.y; ++y)
        {
            for (int x = 0; x < dstShape.x; ++x)
            {
                ...
                float2 mapValue = test::GoldInterp<MI, MB>(map, mapStrides, mapSize, mapBVal, mapAS, mapCoord, z);

                float2 srcCoord = fdstCoord * srcScale + mapValue * valScale + srcOffset;

                test::ValueAt<ValueType>(dst, dstStrides, dstCoord)
                    = test::GoldInterp<SI, SB>(src, srcStrides, srcSize, srcBVal, srcAS, srcCoord, z);
            }
        }
    }
}
...
TYPED_TEST(OpRemap, correct_output)
{
    ...
    using uniform_distribution = std::conditional_t<std::is_integral_v<BT>, std::uniform_int_distribution<BT>,
                                                    std::uniform_real_distribution<BT>>;

    uniform_distribution rand(BT{0}, std::is_integral_v<BT> ? cuda::TypeTraits<BT>::max : BT{1});

    for (int z = 0; z < srcShape.z; ++z)
        for (int y = 0; y < srcShape.y; ++y)
            for (int x = 0; x < srcShape.x; ++x)
                for (int k = 0; k < cuda::NumElements<ValueType>; ++k)
                    cuda::GetElement(test::ValueAt<ValueType>(srcVec, srcStrides, int3{x, y, z}), k) = rand(randEng);
    ...
    for (int z = 0; z < mapShape.z; ++z)
        for (int y = 0; y < mapShape.y; ++y)
            for (int x = 0; x < mapShape.x; ++x)
                test::ValueAt<float2>(mapVec, mapStrides, int3{x, y, z}) = float2{randf(randEng), randf(randEng)};
