                  ...
                  float initSigma, NVCVSIFTFlagType flags, std::optional<Stream> pstream)
{
    ...
    Tensor featCoords      = Tensor::Create({{numSamples, maxFeatures, 4}, "NMC"}, nvcv::TYPE_F32);
    Tensor featMetadata    = Tensor::Create({{numSamples, maxFeatures, 3}, "NMC"}, nvcv::TYPE_F32);
    Tensor featDescriptors = Tensor::Create({{numSamples, maxFeatures, 2}, "NMC"}, nvcv::TYPE_U64);
    Tensor numFeatures     = Tensor::Create({{numSamples, 1}, "NC"}, nvcv::TYPE_S32);
