...
# needed by internal init_repo.sh
apt-get install -y --no-install-recommends pre-commit shellcheck
...
apt-get install -y --no-install-recommends doxygen graphviz python3 python3-pip
